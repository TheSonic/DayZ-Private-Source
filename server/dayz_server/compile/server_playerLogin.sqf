...
diag_log ("PLAYER: LOGIN STARTING: " + str(_this));
...
if ((_playerID == "") or (isNil "_playerID")) exitWith {
	diag_log ("PLAYER: LOGIN FAILED: " + _playerName + " has no login ID");
};

diag_log ("PLAYER: LOGIN ATTEMPT: " + str(_playerID) + " " + _playerName);
...
if (isNull _playerObj or !isPlayer _playerObj) exitWith {
	diag_log ("PLAYER: LOGIN RESULT: Player object null: " + str(_playerObj));
};

if ((_primary select 0) == "ERROR") exitWith {
	diag_log format ["PLAYER: LOGIN RESULT: Failed to load %1 for player: %2 ", _primary, _playerID];
};
...
if (GenderSelect) then { _isNew = count (_primary select 3) == 0; };
...
diag_log ("PLAYER: LOGIN LOADED: " + _playerName + ", Type: " + (typeOf _playerObj));
